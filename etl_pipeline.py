...
# ─── 5a. canonical_test_registry ─────────────────────────────────────────────

CANONICAL_REGISTRY_SQL = """
INSERT IGNORE INTO canonical_test_registry (
    test_name_raw, canonical_name, si_unit, si_factor
) VALUES (%s,%s,%s,%s)
"""

def load_canonical_test_registry(cursor, rows: list) -> int:
    data = []
    for r in rows:
        data.append((
    ...
    return _exec_many(cursor, CANONICAL_REGISTRY_SQL, data)
...
INSERT INTO test_limits (
    file_id, lot_id, test_num, test_name_raw,
    lo_limit_scaled, hi_limit_scaled, orig_unit,
...
def run_etl_for_file(
    ...
    try:
        ...
        log.info("  Loading canonical_test_registry …")
        counts["canonical_test_registry"] = load_canonical_test_registry(cursor, result.get("canonical_test_registry", []))
